<template>
	<div class="h-full">
		<n-card title="权限切换" :bordered="false" class="h-full rounded-8px shadow-sm">
			<div class="pb-12px">
				<n-gradient-text type="primary" :size="20">当前用户的权限：{{ auth.userInfo.roles }}</n-gradient-text>
			</div>
			<n-select
				:value="auth.userInfo.roles"
				class="w-120px"
				size="small"
				:options="options"
			...
			<div>
				<n-button v-permission="'announcementProject'" class="mr-12px">announcementProject可见</n-button>
