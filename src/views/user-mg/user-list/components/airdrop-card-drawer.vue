<template>
  ...
  >
    <n-drawer-content title="空投卡牌" class="w-full">
      <n-layout>
...
"
            ...
            <n-form-item-grid-item :span="24" label="卡牌数量" path="quantity">
              <n-input v-model:value="formModel.quantity" />
            </n-form-item-grid-item>

            <n-form-item-grid-item :span="24" label="卡牌id" path="collections">
              <n-input v-model:value="formModel.collections" />
            </n-form-item-grid-item>
          </n-grid>
...
import { api_airdrop_card } from '@/api'
import type { DrawerPlacement, FormInst, FormItemRule } from 'naive-ui'
import { computed, ref, watch } from 'vue'

export type FormModel = ApiUser.AirDropCard
...
const confirm = async () => {
  ...
  formRef.value?.validate(async err => {
    ...
    const res = await api_airdrop_card(formModel.value)
    console.log(res)
    if (res.code === 200) {
