<template>
  ...
  >
    <n-drawer-content title="空投积分" class="w-full">
      <n-layout>
...
"
            ...
            <n-form-item-grid-item :span="24" label="积分数量" path="credits_num">
              <n-input v-model:value="formModel.credits_num" />
            </n-form-item-grid-item>

            <!-- <n-form-item-grid-item :span="24" label="积分id" path="credits_num">
              <n-input v-model:value="formModel.credits_num" />
            </n-form-item-grid-item> -->
          </n-grid>
...
import { api_airdrop_user_credits } from '@/api'
import type { DrawerPlacement, FormInst, FormItemRule } from 'naive-ui'
import { computed, ref, watch } from 'vue'

export type FormModel = ApiUser.AirDropcreditsProps
...
const confirm = async () => {
  ...
  formRef.value?.validate(async err => {
    ...
    const res = await api_airdrop_user_credits(formModel.value)
    console.log(res)
    if (res.code === 200) {
